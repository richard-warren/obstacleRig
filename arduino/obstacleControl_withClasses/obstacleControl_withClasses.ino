...
// speed measurements
MovingObject wheel = MovingObject((2*PI*wheelRad)/encoderSteps, wheelSpeedDistance);
MovingObject motor = MovingObject((2*PI*timingPulleyRad)/(motorSteps*microStepping), obsSpeedDistance);

// unit conversions
const double motorTicsPerWheelTic = wheel.mPerTic / motor.mPerTic; // stepper motor tics per tic of wheel rotary encoder
...
void setup() {
  ...
  calibrateLimits();
  wheel.setObjectSpeed(.01);
  motor.setObjectSpeed(obsSpeedStart);
...
void loop(){
  ...
  // obstacle engage
  if (isObsOn && !isObsTracking && (wheelTicsTemp>=(obsLocation/wheel.mPerTic))){
  ...
  // obstacle disengage
  else if (isObsTracking && motorTics>=((trackEndPosition-obsStopPos)/motor.mPerTic)){
    ...
    takeSteps(max((obsStartPos+getJitter(obsStartPosJitter))/motor.mPerTic,0), ACCELERATE, obsSpeedStart, callibrationSpeed);  // moving back to start position
    digitalWrite(motorOffPin, HIGH);
  ...
  // water delivery
  else if (wheelTicsTemp >= (waterDistance/wheel.mPerTic)){
    giveWater();
    Serial.print(wheel.getSpeed(),3); Serial.println();
  }
  ...
  // obstacle position update
    ...
    targetMotorTics = constrain(targetMotorTics, 0, (trackEndPosition-obsStopPos)/motor.mPerTic+1) - motorTics;  // +1 is needed because the motor tics must EXCEED the threshold for the obstacle to be disengaged
