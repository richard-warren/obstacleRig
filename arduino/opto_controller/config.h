...
volatile int signalDuration = 10000; // ms (includes ramp up and ramp down times) // also serves as the max stimulus time when stim is externally triggered
...
volatile float signalProbability = .25; // probability of delivering stimulus when trigger is received
volatile float signalPower = .01; // fraction of light power
const float signalPowers[] = {0.25, 0.5, 1.0}; // randomly select among these powers ONLY WHEN STIMULUS IS EXTERNALLY TRIGGERED AND randomizeTriggeredPower IS TRUE
