...
// settings
const int laserPin = 2; // must be either 2 or 3 (these are interrupt pins on arduino)
const float hz = 40; // frequency of sine wave
const int maxLightTime = 3000; // ms
const int laserExpDuration = 1000; // ms
const int rampDownTime = 100; // ms
const int rampUpTime = 100; // ms
const int testStimDuration = 1000; // ms
volatile bool stopWhenPinLow = false; // should you discontinue light when the triggering pin goes low?
volatile float lightProbability = 1;
volatile float lightPower = .01;
volatile bool isLightPowerRandom = true;
volatile float randomLightPower = 0.01;
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(laserPin), lightOnOff, CHANGE);
  Serial.begin(115200);
  showMenu();
...
void loop(){
  ...
  if (Serial.available()){
    ...
    switch (userInput){
      ...
      case 2:
        if(isLightPowerRandom){
          lightPower = 0.01;
          isLightPowerRandom = false;
        }else{
          lightProbability = 1;
          isLightPowerRandom = true;
        }
        updated = true;
        showMenu();
        break;
      ...
      case 3:
        if(isLightPowerRandom){
          Serial.println("Users should not set the light power when it's been set to random");
        }else{
...
void lightOnOff(){
  if ((digitalRead(laserPin) && random(0,100)<(lightProbability*100.0))){
    if(isLightPowerRandom){
      randomLightPower = random(6,13);
      lightPower = randomLightPower/50;
    }
    deliverLight(laserExpDuration);
...
void showMenu(){
  ...
  if (isLightPowerRandom){
    Serial.println("2: light power has been set to random");
    Serial.println("3: light power cannot be set manually");
  }else{
    Serial.println("2: light power is NOT set to random");
    Serial.print("3: set light power: ");
    Serial.println(lightPower, 3);
  }
  
  Serial.print("4: set light probability: ");
  Serial.println(lightProbability);
  if (constantLightOn){
    Serial.println("5: turn constant light OFF");
  }else{
    Serial.println("5: turn constant light ON");
  }
  Serial.println("6: step pulse");
}
